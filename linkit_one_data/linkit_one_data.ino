...
#define per 50
#define per1 3
#define DEVICEID "DAhnGAC7" // Input your deviceId
#define DEVICEKEY "GAnr08h5z0m079qm" // Input your deviceKey
...
int x=0,y=0,z=0,light=0;
String upload_Y,upload_Light;
...
void uploadstatus(){
  ...
  sprintf(bufferY,"%d",y);
  sprintf(bufferLight,"%d",light);

  upload_Y = "Acc,,"+String(bufferY);
  upload_Light = "Light,,"+String(bufferLight);
  ...
  delay(100);
  String uploadData = upload_Y+"\n"+upload_Light;
  int thislength = uploadData.length();
  HttpClient http(c2);
